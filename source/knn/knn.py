...
class KNN:
    ...
    In the predict method, the distances between each data point in X and all training examples in X_train are calculated using the Euclidean distance formula. The indices of the k nearest neighbors are then obtained using np.argsort to sort the distances in ascending order and select the indices of the k smallest distances. The corresponding labels of the nearest neighbors are then extracted from y_train. Finally, the predicted label for each data point in X is determined by taking the mode of the nearest neighbor labels using np.bincount and argmax.

    Note that this implementation assumes that X and X_train are numpy arrays with the same number of columns/features. Additionally, the labels in y and the predicted labels returned by the predict method are assumed to be integer values representing the class labels.
    """
    def __init__(self):
        self.X_train = None
        self.y_train = None
    ...
    def predict(self, k: int, X: np.array) -> np.array:
        y_pred = []
    ...
    def predict(self, X):
        """
In this implementation, we take advantage of numpy's ability to perform operations on arrays element-wise to vectorize the calculation of distances between X and self.X_train.

First, we reshape X and self.X_train using np.newaxis to create a new axis so that we can perform broadcasting, which allows numpy to perform operations on arrays with different shapes.
